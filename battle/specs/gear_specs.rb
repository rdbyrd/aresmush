    ...
    describe Battle do
      ...
      describe :armor_stat do
        before do
          ...
          allow(Battle).to receive(:armor_specials) { specials }
          allow(Battle).to receive(:armor).with("Military") { { "defense" => 1, "protection" => { "Head" => 1, "Body" => 4 } } }
        end

        it "should return nil if armor not found" do
          allow(Battle).to receive(:armor).with("Police") { nil }
          expect(Battle.armor_stat("Police", "protection")).to be_nil
        end

        it "should add special protection together" do
          protection = Battle.armor_stat("Military+Helmet+Cup", "protection")
        ...
        it "should add other stats" do
          defense = Battle.armor_stat("Military+Helmet", "defense")
          expect(defense).to eq 3
        end

        it "should not add anything if no special stat exists" do
          defense = Battle.armor_stat("Military+Cup", "defense")
      ...
      describe :weapon_stat do
        before do
          allow(Battle).to receive(:weapon).with("Rifle") { { "penetration" => 3, "lethality" => 10 } }
          specials = {
            "Ap" => { "penetration" => 2 },
            "Silencer" => { "lethality" => -5 }
          }
          allow(Battle).to receive(:weapon_specials) { specials }
        end

        it "should return nil if weapon not found" do
           allow(Battle).to receive(:weapon) { nil }
           expect(Battle.weapon_stat("Banjo", "penetration")).to be_nil
        end

        it "should return nil if weapon has no stat" do
           expect(Battle.weapon_stat("Rifle", "moxie")).to be_nil
        end

        it "should return a value for a weapon without a special" do
          expect(Battle.weapon_stat("Rifle", "penetration")).to eq 3
        end

        it "should return raw value for a weapon with an invalid special" do
          expect(Battle.weapon_stat("Rifle+Foo", "penetration")).to eq 3
        end

        it "should adjust a value for a weapon with a special" do
          expect(Battle.weapon_stat("Rifle+Ap", "penetration")).to eq 5
        end

        it "should handle multiple specials" do
          expect(Battle.weapon_stat("Rifle+Ap+Silencer", "penetration")).to eq 5
        end
      end
      ...
      describe :set_default_gear do
        ...
        it "should set a weapon if set" do
          expect(Battle).to receive(:combatant_type_stat).with("soldier", "weapon") { "rifle" }
          expect(Battle).to receive(:combatant_type_stat).with("soldier", "weapon_specials") { "specials" }
          expect(Battle).to receive(:set_weapon).with(@enactor, @combatant, "rifle", "specials")
          Battle.set_default_gear(@enactor, @combatant, "soldier")
        end

        it "should not set a weapon if none set" do
          expect(Battle).to_not receive(:set_weapon)
          Battle.set_default_gear(@enactor, @combatant, "soldier")
        end

        it "should set armor if set" do
          expect(Battle).to receive(:combatant_type_stat).with("soldier", "armor") { "kevlar" }
          expect(Battle).to receive(:set_armor).with(@enactor, @combatant, "kevlar")
          Battle.set_default_gear(@enactor, @combatant, "soldier")
        end

        it "should not set armor if none set" do
          expect(Battle).to_not receive(:set_armor)
          Battle.set_default_gear(@enactor, @combatant, "soldier")
        end
      ...
      describe :set_weapon do
        before do
          ...
          allow(Battle).to receive(:npcmaster_text) { "npcmaster" }
        ...
        it "should pretty up the weapon and specials if set" do
          ...
          Battle.set_weapon(@enactor, @combatant, "rifle", [ "s1", "s2" ])
        end

        it "should reset ammo" do
          expect(Battle).to receive(:weapon_stat).with("rifle", "ammo") { 22 }
        ...
        it "should reset stats if nil" do
          ...
          Battle.set_weapon(@enactor, @combatant, nil, nil)
        end
        ...
        it "should emit to combat" do
          expect(Battle).to receive(:emit_to_combat).with(@combat, "battle.weapon_changed", "npcmaster")
          Battle.set_weapon(@enactor, @combatant, "rifle", [ "s1", "s2" ])
        end
      end
    end
