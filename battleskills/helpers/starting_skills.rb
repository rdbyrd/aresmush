module AresMUSH
  module BattleSkills
    module StartingSkills
      def self.config
        Global.read_config("battleskills", "starting_skills") || {}
      end
      
      def self.get_skills_for_char(char)
        skills = {}
        
        BattleSkills.attrs.map { |a| a['name'] }.each do |a|
          skills[a] = 1
        end
        
        BattleSkills.action_skills.map { |a| a['name'] }.each do |a|
          skills[a] = Global.read_config('battleskills', 'allow_unskilled_action_skills') ? 0 : 1
        end
