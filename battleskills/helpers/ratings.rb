module AresMUSH
  module BattleSkills
    ...
    def self.get_linked_attr(ability_name)
      case BattleSkills.get_ability_type(ability_name)
      when :action
        config = BattleSkills.action_skill_config(ability_name)
        return config["linked_attr"]
      when :attribute
        return nil
      else
        return Global.read_config("battleskills", "default_linked_attr")
      end
    end
