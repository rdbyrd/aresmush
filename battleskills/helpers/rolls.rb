module AresMUSH
  module BattleSkills
    ...
    def self.roll_ability(char, roll_params)
      dice = BattleSkills.dice_to_roll_for_ability(char, roll_params)
    ...
    def self.get_success_level(die_result)
      successes = die_result.count { |d| d >= BattleSkills.success_target_number }
    ...
    def self.emit_results(message, client, room, is_private)
      if (is_private)
        ...
        room.emit message
        channel = Global.read_config("battleskills", "roll_channel")
        if (channel)
