module AresMUSH
  module BattleSkills
    ...
    def self.dice_to_roll_for_ability(char, roll_params)
      ...
      linked_attr = roll_params.linked_attr || BattleSkills.get_linked_attr(ability)
      ability_type = BattleSkills.get_ability_type(ability)
      skill_rating = BattleSkills.ability_rating(char, ability)
      ...
      apt_rating = linked_attr ? BattleSkills.ability_rating(char, linked_attr) : 0
    ...
    def self.parse_and_roll(char, roll_str)
      if (roll_str.is_integer?)
        ...
        die_result = BattleSkills.roll_dice(dice)
      else
        roll_params = BattleSkills.parse_roll_params roll_str
        if (!roll_params)
          return nil
        end
        die_result = BattleSkills.roll_ability(char, roll_params)
      end
      die_result
    end
