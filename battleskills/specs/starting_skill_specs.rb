    ...
    describe BattleSkills do 
      ...
      describe :get_skills_for_char do
        before do 
          ...
          allow(Global).to receive(:read_config).with("battleskills", "starting_skills") { starting_skills }
          allow(Global).to receive(:read_config).with("battleskills", "attributes") { [ ] }
          allow(Global).to receive(:read_config).with("battleskills", "action_skills") { [ ] }
        ...
        it "should include default attributes" do
          allow(@char).to receive(:group).with("Position") { "Other" }
          allow(@char).to receive(:group).with("Faction") { "Other" }
          allow(Global).to receive(:read_config).with("battleskills", "attributes") { [ { 'name' => 'Brawn' }, { 'name' => 'Presence' } ] }
        ...
        it "should include default action skills when unskilled allowed" do
          allow(@char).to receive(:group).with("Position") { "Other" }
          allow(@char).to receive(:group).with("Faction") { "Other" }
          allow(Global).to receive(:read_config).with("battleskills", "action_skills") { [ { 'name' => 'Melee' } ] }
          allow(Global).to receive(:read_config).with("battleskills", "allow_unskilled_action_skills") { true }
