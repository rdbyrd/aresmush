module AresMUSH
  module BattleSkills
    describe BattleSkills do
      ...
      before do
        allow(Global).to receive(:read_config).with("battleskills", "free_languages") { 3 }
      end
      ...
      describe :points_on_background do
        before do
          @char = double
          allow(Global).to receive(:read_config).with("battleskills", "free_backgrounds") { 5 }
        end
      ...
      describe :points_on_language do
        before do
          @char = double
          allow(Global).to receive(:read_config).with("battleskills", "free_languages") { 4 }
        end
