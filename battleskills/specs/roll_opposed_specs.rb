module AresMUSH
  module BattleSkills
    describe OpposedRollCommand do
      describe :crack do
        it "should fail if the string is invalid" do
          handler = OpposedRollCommand.new(nil, Command.new("roll A vs B"), nil)
          handler.parse_args
        ...
        it "should crack a PC roll vs a NPC number" do
          handler = OpposedRollCommand.new(nil, Command.new("roll A/1 vs 3"), nil)
          handler.parse_args
        ...
        it "should crack a PC roll vs another PC" do
          handler = OpposedRollCommand.new(nil, Command.new("roll A/Firearms vs B/Melee"), nil)
          handler.parse_args
          expect(handler.name1).to eq "A"
        ...
        it "should crack skill names with spaces" do
          handler = OpposedRollCommand.new(nil, Command.new("roll A/Basket Weaving+2 vs B/Scuba Diving - 1"), nil)
          handler.parse_args
          expect(handler.name1).to eq "A"
          expect(handler.name2).to eq "B"
