module AresMUSH
  ...
  module BattleSkills
    ...
    def self.is_enabled?
      !Global.plugin_manager.is_disabled?("battleskills")
    end
    ...
    def self.one_shot_roll(char, roll_params)
      roll = BattleSkills.roll_ability(char, roll_params)
    ...
    def self.one_shot_die_roll(dice)
      roll = BattleSkills.roll_dice(dice)
      roll_result = BattleSkills.get_success_level(roll)
      success_title = BattleSkills.get_success_title(roll_result)
    ...
    def self.app_review(char)
      text = BattleSkills.total_point_review(char)
      text << "%r"
      text << BattleSkills.ability_rating_review(char)
      text << "%r"
      text << BattleSkills.backgrounds_review(char)
      text << "%r%r"
      text << BattleSkills.starting_language_review(char)
      text << "%r"
      text << BattleSkills.starting_skills_check(char)
      text << "%r"
      text << BattleSkills.unusual_skills_check(char)
      text
    end
    ...
    def self.dice_rolled(char, ability)
      BattleSkills.dice_to_roll_for_ability(char, RollParams.new(ability))
    end    
    
    def self.save_char(char, chargen_data)      
      (chargen_data[:fs3][:fs3_attributes] || []).each do |k, v|
        error = BattleSkills.set_ability(char, k, v.to_i)
        return t('battleskills.error_saving_ability', :name => k, :error => error) if error
      end
      ...
      (chargen_data[:fs3][:fs3_action_skills] || []).each do |k, v|
        ...
        ability = BattleSkills.find_ability(char, k)
        if (ability)
    ...
    def self.luck_for_scene(char, scene)
      ...
      luck_config = Global.read_config('battleskills', 'luck_for_scene') || {}
