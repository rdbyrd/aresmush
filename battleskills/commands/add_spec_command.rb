  ...
  module BattleSkills
    class AddSpecialtyCommand
      ...
      def check_name_for_dots
        return t('battleskills.no_special_characters') if (self.specialty !~ /^[\w\s]+$/)
        return nil
      end
      ...
      def handle
        ClassTargetFinder.with_a_character(self.target, client, enactor) do |model|        
          ...
          if (!config['specialties'].include?(self.specialty))     
            client.emit_failure t('battleskills.invalid_specialty', :names => config['specialties'].join(", "))
            return
          end
        
          if (ability.specialties.include?(self.specialty))
            client.emit_failure t('battleskills.specialty_already_exists', :name => model.name)
            return
          end
          ...
          client.emit_success t('battleskills.specialty_added', :name => self.specialty)
        end
      end
    end
  end
